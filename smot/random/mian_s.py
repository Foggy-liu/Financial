...
def train_random_forest(X_train, y_train, X_test, y_test, cv_folds=5):
    """
    使用随机森林进行训练
    """
    print("\n" + "=" * 70)
    print(f"随机森林训练 - {cv_folds}折交叉验证")
    print("=" * 70)
    ...
    print("\n开始网格搜索...")
    total_combos = len(n_estimators_list) * len(max_depth_list) * len(min_samples_split_list)
    print(
        f"参数组合: {len(n_estimators_list)}个树数量 × {len(max_depth_list)}个深度 × {len(min_samples_split_list)}个分裂条件 = {total_combos}种")
    print("-" * 50)
    ...
    for n_estimators in n_estimators_list:
        for max_depth in max_depth_list:
            for min_samples_split in min_samples_split_list:
                ...
                print(
                    f"  n={n_estimators}, depth={depth_str}, split={min_samples_split}: AUC={mean_auc:.4f}, F1={mean_f1:.4f}")
    ...
    # 修复：将 nan 转换为 None（如果是空值）
    max_depth_value = best['max_depth']
    ...
    print(f"  max_depth = {max_depth_value}")
...
def main():
    ...
    # 2. 训练随机森林
    model, results, test_auc, test_f1, test_acc = train_random_forest(
    ...
    plt.title('混淆矩阵 - 随机森林 (SMOTE后)', fontsize=14)
    ...
    print(f"\n混淆矩阵已保存为 'rf_confusion_matrix.png'")

    # 4. 保存模型
    # joblib.dump(model, 'best_random_forest_model_smote.pkl')
    # print(f"模型已保存为 'best_random_forest_model_smote.pkl'")

    # # 5. 保存结果
    # results.to_csv('rf_tuning_results.csv', index=False)
    # print("调优结果已保存为 'rf_tuning_results.csv'")

    print("\n" + "=" * 70)
    print("随机森林训练完成！")
    print("=" * 70)
    print(
        f"最佳参数: n_estimators={model.n_estimators}, max_depth={model.max_depth}, min_samples_split={model.min_samples_split}")
